class LeaveRecord < ApplicationRecord
  ...
  def no_overlap
    overlapping_leaves = LeaveRecord.where(user_id:)
                                    .where.not(id:) # Exclude the current record if it's already saved
  ...
  def no_duplicate_booking
    existing_record = LeaveRecord.where(user_id:, start_date:, end_date:).exists?
